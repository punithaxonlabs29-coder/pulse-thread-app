...
export class DatabaseService {
  ...
  public static async init(): Promise<void> {
    // Already initialized
    if (this.db) return;
    // Init already in progress — wait for it instead of opening a second connection
    ...
    this._initPromise = (async () => {
      try {
        ...
        // dramatically reducing "database table is locked" errors at startup.
        await this.db.execAsync('PRAGMA journal_mode = WAL;');
        // busy_timeout: if another connection holds the lock, wait up to 5 s
        // before throwing instead of failing immediately.
  ...
  private static async setupSchema(): Promise<void> {
    ...
    // Run ALL DDL inside ONE exclusive transaction.
    // Previously, each statement was a separate runAsync() call — if multiple
    // callers hit init() concurrently they'd race on the same tables.
    try {
